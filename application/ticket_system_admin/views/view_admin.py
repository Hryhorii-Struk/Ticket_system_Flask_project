...
from application.ticket_system_admin.forms.forms_admin import AddGroupForm
from application.ticket_system_admin.forms.forms_admin import AddUserForm
from application.ticket_system_admin.forms.forms_admin import EnterPasswordForm
from application.ticket_system_admin.forms.forms_admin import EditUserForm
from . import admin_bp

principals = Principal(app)
# define ticket_system_admin role need
admin_only = RoleNeed('ticket_system_admin')
...
def delete_user():
    ...
    # we won't ever delete the ticket_system_admin user (id = 1)
    if id == '1':
        flash(gettext('Can\'t delete default ticket_system_admin user.'), category='warning')
        return redirect(url_for('admin_bp.index'))
...
def admin_edit_group():
    ...
    # prevent editing of ticket_system_admin group name as this is hard coded into ticket_system_admin view permissions.
...
def admin_delete_group():
    ...
    # we won't ever delete the ticket_system_admin group (id = 1)
    if id == '1':
        flash(gettext('Can\'t delete default ticket_system_admin group.'), category='warning')
        return redirect(url_for('admin_bp.index'))
